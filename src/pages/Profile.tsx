...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    ...
    if (data && !error) {
      setProfile({
        ...
        allergies: (data.allergies as string[]) || [], food_preferences: data.food_preferences || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      city: profile.city, postal_code: profile.postal_code, allergies: profile.allergies,
      food_preferences: profile.food_preferences, favorite_table_area: profile.favorite_table_area,
    }).eq('user_id', user!.id);
    if (error) { toast.error(t('profile.saveError')); } else { toast.success(t('profile.saveSuccess')); }
    setLoading(false);
  };
  ...
  if (authLoading) {
    return (<div className="min-h-screen bg-background flex items-center justify-center"><p className="font-body text-muted-foreground">{t('profile.loadingText')}</p></div>);
  }
        ...
        <div className="max-w-xl mx-auto">
          ...
          <div className="bg-card rounded-lg p-8 shadow-lg border border-border">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block font-body text-sm font-bold text-foreground mb-1.5">{t('profile.fullName')}</label>
              ...
              <div>
                <label className="block font-body text-sm font-bold text-foreground mb-1.5">{t('profile.address')}</label>
              ...
              <div>
                <label className="block font-body text-sm font-bold text-foreground mb-2">{t('profile.allergies')}</label>
                <div className="flex flex-wrap gap-2">
                  {EU_ALLERGENS.map((allergen) => {
                    const isSelected = profile.allergies.includes(allergen.id);
                    return (
                      <button key={allergen.id} type="button"
                        onClick={() => { setProfile(prev => ({ ...prev, allergies: isSelected ? prev.allergies.filter(a => a !== allergen.id) : [...prev.allergies, allergen.id] })); }}
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-md border text-sm font-body font-bold transition-all ${
                          isSelected ? 'bg-destructive/15 border-destructive text-destructive ring-1 ring-destructive/30' : 'bg-card border-border text-muted-foreground hover:border-foreground/30'
                        }`}>
              ...
              <div>
                <label className="block font-body text-sm font-bold text-foreground mb-1.5">{t('profile.foodPreferences')}</label>
                <textarea name="food_preferences" value={profile.food_preferences}
                  onChange={(e) => setProfile(prev => ({ ...prev, food_preferences: e.target.value }))} rows={2}
              ...
              <div>
                <label className="block font-body text-sm font-bold text-foreground mb-1.5">{t('profile.favoriteArea')}</label>
                <input type="text" name="favorite_table_area" value={profile.favorite_table_area} onChange={handleChange}
                  className="w-full px-4 py-3 rounded-sm bg-background border border-input font-body text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
